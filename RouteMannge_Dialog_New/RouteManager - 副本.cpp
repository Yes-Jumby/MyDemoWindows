...
void CRouteManager::FunThreadRouteRunBack() //后运行
{
    if(m_bIsThreadContinueRun)
    {
        ...
        m_MotionCard.CoordinateLineMove(1,m_FrontGrat + m_Long/2 + 10000,20000,0,m_iLineVel,m_iLineAcc);//偏置2W以规避
        m_MotionCard.CoordinateStart(1);
        m_MotionCard.CoordinateMovedForXR(1);
...
//            }
        ...
        //运动到夹点
        m_MotionCard.CoordinateLineMove(1,2885,20000,0,m_iLineVel,m_iLineAcc);//偏置2W以规避
        m_MotionCard.CoordinateLineMove(1,2885,-3520,0,m_iLineVel,m_iLineAcc);//夹点需要实际验证
        m_MotionCard.CoordinateStart(1);
        m_MotionCard.CoordinateMovedForXR(1);
...
bool CRouteManager::IPhonePath_1()//正面检测
{
    ...
    //正面同轴、补光
    flag = m_MotionCard.CoordinateBufIO(1,0x0040,0x0000);//开启正背同轴光
	flag = m_MotionCard.CoordinateBufIO(1, 0x0200, 0x0000);//触发正背检测补光
    flag = m_MotionCard.CoordinateLineMove(1,m_p1-m_Long/2,m_FrontBackYOffSet,m_FrontBackDDROffSet,m_iFrontVel,m_iFrontAcc);//
    flag = m_MotionCard.CoordinateBufIO(1,0x0001,0x0000);//触发正背检测相机
    ...
    flag = m_MotionCard.CoordinateBufIO(1,0x0241,0xFFFF);//关闭 正背检测相机触发 正背检测同轴光、补光

    //低角度
    flag = m_MotionCard.CoordinateBufIO(1,0x0080,0x0000);//触发正背检测低角度光1
    flag = m_MotionCard.CoordinateBufIO(1,0x0100,0x0000);//触发正背检测低角度光2
    flag = m_MotionCard.CoordinateLineMove(1,m_p1-m_Long/2 + 12*m_FrontBackImageHeight,m_FrontBackYOffSet,m_FrontBackDDROffSet,m_iFrontVel,m_iFrontAcc);
    flag = m_MotionCard.CoordinateBufIO(1,0x0001,0x0000);//触发 正背检测相机触发
    flag = m_MotionCard.CoordinateLineMove(1,m_p1-m_Long/2,m_FrontBackYOffSet,m_FrontBackDDROffSet,m_iFrontVel,m_iFrontAcc);//
    flag = m_MotionCard.CoordinateBufIO(1,0x0181,0xFFFF);//关闭 正背检测相机 低角度光1 低角度光2 补光 触发
...
//    //走到中框
//    flag = m_MotionCard.CoordinateLineMove(1, m_p3 - (m_Long / 2 + 2*m_FrameStep), m_FrameOffset, 0, m_iLineVel, m_iLineAcc);
//    //第一边
//    for (int i = 0; i < StepLong; i++)
//    {
//        flag = m_MotionCard.CoordinateLineMove(1, m_p3 - m_Long / 2 + (2 * i + 1)*m_FrameStep, m_FrameOffset + 0, 0, m_iFrameVel, m_iFrameAcc);
//        flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0x0000);//触发中框拍照系统
//        flag = m_MotionCard.CoordinateLineMove(1, m_p3 - m_Long / 2 + (2 * i + 2)*m_FrameStep, m_FrameOffset + 0, 0, m_iFrameVel, m_iFrameAcc);
//        flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0xFFFF);//关闭中框拍照系统
//    }
//    //第一个R角
//    //flag = m_MotionCard.CoordinateLineMove(1,m_p3 + m_Long/2 - LongOffsetX,m_FrameOffset-LongOffsetY,-m_Circle14/2,vel,acc);
//    flag = m_MotionCard.CoordinateLineMove(1, 685720, -16661, -m_Circle14 / 2, m_iFrameVel, m_iFrameAcc);
//    flag = m_MotionCard.CoordinateBufDelay(1,10);
//    flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0x0000);//触发中框拍照系统
//    flag = m_MotionCard.CoordinateBufDelay(1, 1);//延迟1ms
//    flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0xFFFF);//关闭中框拍照系统

//    //第二边
//    flag = m_MotionCard.CoordinateLineMove(1, m_p3 - (m_Width / 2 + 2*m_FrameStep), m_FrameOffset - m_W_H, -m_Circle14, m_iFrameVel, m_iFrameAcc);
//    flag = m_MotionCard.CoordinateBufDelay(1,100);
//    for (int i = 0; i < StepWidth; i++)
//    {
//        flag = m_MotionCard.CoordinateLineMove(1, m_p3 - m_Width / 2 + 3600 + (2 * i + 1)*m_FrameStep, m_FrameOffset - m_W_H, -m_Circle14, m_iFrameVel, m_iFrameAcc);
//        flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0x0000);//触发中框拍照系统
//        flag = m_MotionCard.CoordinateLineMove(1, m_p3 - m_Width / 2 + 3600 + (2 * i + 2)*m_FrameStep, m_FrameOffset - m_W_H, -m_Circle14, m_iFrameVel, m_iFrameAcc);
//        flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0xFFFF);//关闭中框拍照系统
//    }
//    //第二个R角20000+2610
...
//    flag = m_MotionCard.CoordinateLineMove(1, 631395, -16863, -m_Circle14 - m_Circle14 / 2, m_iFrameVel, m_iFrameAcc);
//    flag = m_MotionCard.CoordinateBufDelay(1,10);
//    flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0x0000);//触发中框拍照系统
//    flag = m_MotionCard.CoordinateBufDelay(1, 1);//延迟1ms
//    flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0xFFFF);//关闭中框拍照系统
...
//    for (int i = 0; i < StepLong; i++)
//    {
//        flag = m_MotionCard.CoordinateLineMove(1, m_p3 - m_Long / 2 + (2 * i + 1)*m_FrameStep, m_FrameOffset + 0, -2 * m_Circle14, m_iFrameVel, m_iFrameAcc);
//        flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0x0000);//触发中框拍照系统
//        flag = m_MotionCard.CoordinateLineMove(1, m_p3 - m_Long / 2 + (2 * i + 2)*m_FrameStep, m_FrameOffset + 0, -2 * m_Circle14, m_iFrameVel, m_iFrameAcc);
//        flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0xFFFF);//关闭中框拍照系统
//    }
//    //第三个R角
...
//    flag = m_MotionCard.CoordinateLineMove(1, 685620, -18561, -2 * m_Circle14 - m_Circle14 / 2, m_iFrameVel, m_iFrameAcc);
//    flag = m_MotionCard.CoordinateBufDelay(1,10);
//    flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0x0000);//触发中框拍照系统
//    flag = m_MotionCard.CoordinateBufDelay(1, 1);//延迟1ms
//    flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0xFFFF);//关闭中框拍照系统
...
//    //第四边
//    flag = m_MotionCard.CoordinateLineMove(1, m_p3 - (m_Width / 2 + 2*m_FrameStep), m_FrameOffset - m_W_H, -3 * m_Circle14, m_iFrameVel, m_iFrameAcc);
//    flag = m_MotionCard.CoordinateBufDelay(1,100);
//    for (int i = 0; i < StepWidth; i++)
//    {
//        flag = m_MotionCard.CoordinateLineMove(1, m_p3 - m_Width / 2 + 3600 + (2 * i + 1)*m_FrameStep, m_FrameOffset - m_W_H, -3 * m_Circle14, m_iFrameVel, m_iFrameAcc);
//        flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0x0000);//触发中框线阵 相机1 相机2
//        flag = m_MotionCard.CoordinateLineMove(1, m_p3 - m_Width / 2 + 3600 + (2 * i + 2)*m_FrameStep, m_FrameOffset - m_W_H, -3 * m_Circle14, m_iFrameVel, m_iFrameAcc);
//        flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0xFFFF);//关闭触发中框线阵 相机1、相机2、光源
//    }
//    //第四个R角
...
//    flag = m_MotionCard.CoordinateLineMove(1, 631380, -19260, -3 * m_Circle14 - m_Circle14 / 2, m_iFrameVel, m_iFrameAcc);
//    flag = m_MotionCard.CoordinateBufDelay(1,10);
//    flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0x0000);//触发中框拍照系统
//    flag = m_MotionCard.CoordinateBufDelay(1, 1);//延迟1ms
//    flag = m_MotionCard.CoordinateBufIO(1, 0x0800, 0xFFFF);//关闭中框拍照系统
...
//    return flag;
    ...
    bool flag;
    short sPlusTime = 500;
    int StepLong = m_Long/m_FrameStep / 2 + 1;//9
    int StepWidth = m_Width/m_FrameStep / 2;//5
    //设置IO口

    flag = m_MotionCard.Compare2DClear(0);
    flag = m_MotionCard.Compare2DMode(0,1);//mode：0为1D，1为2D
    ...
    My2DCompareData MyCompareData[30];
//    for(int i =0;i<30;i++)
//    {
//        MyCompareData[i].px=0;
//    }
    //第一边数据
    MyCompareData[0].py=2*m_FrameStep;
    MyCompareData[1].py=4*m_FrameStep;
    MyCompareData[2].py=6*m_FrameStep;
    MyCompareData[3].py=8*m_FrameStep;
    MyCompareData[4].py=10*m_FrameStep;
    MyCompareData[5].py=12*m_FrameStep;
    MyCompareData[6].py=14*m_FrameStep;
    MyCompareData[7].py=16*m_FrameStep;
    MyCompareData[8].py=18*m_FrameStep;

    MyCompareData[0].px=0;
    MyCompareData[1].px=0;
    MyCompareData[2].px=0;
    MyCompareData[3].px=0;
    MyCompareData[4].px=0;
    MyCompareData[5].px=0;
    MyCompareData[6].px=0;
    MyCompareData[7].px=0;
    MyCompareData[8].px=0;

    //R角
    MyCompareData[9].py=685720 - (m_p3-(m_Long/2+m_FrameStep));
    MyCompareData[9].px=-m_Circle14/2;

    //第二边数据
    MyCompareData[10].py=3600 + m_W_H +2*m_FrameStep;
    MyCompareData[11].py=3600 + m_W_H +2*m_FrameStep;
    MyCompareData[12].py=3600 + m_W_H +4*m_FrameStep;
    MyCompareData[13].py=3600 + m_W_H +6*m_FrameStep;


    MyCompareData[10].px=-m_Circle14;
    MyCompareData[11].px=-m_Circle14;
    MyCompareData[12].px=-m_Circle14;
    MyCompareData[13].px=-m_Circle14;

    //R角
    MyCompareData[14].py=631395 - (m_p3-(m_Long/2+m_FrameStep));
    MyCompareData[14].px=-m_Circle14-m_Circle14/2;

    //第三边数据
    MyCompareData[15].py=2*m_FrameStep;
    MyCompareData[16].py=4*m_FrameStep;
    MyCompareData[17].py=6*m_FrameStep;
    MyCompareData[18].py=8*m_FrameStep;
    MyCompareData[19].py=10*m_FrameStep;
    MyCompareData[20].py=12*m_FrameStep;
    MyCompareData[21].py=14*m_FrameStep;
    MyCompareData[22].py=16*m_FrameStep;
    MyCompareData[23].py=18*m_FrameStep;

    MyCompareData[15].px= -2*m_Circle14;
    MyCompareData[16].px=-2*m_Circle14;
    MyCompareData[17].px=-2*m_Circle14;
    MyCompareData[18].px=-2*m_Circle14;
    MyCompareData[19].px=-2*m_Circle14;
    MyCompareData[20].px=-2*m_Circle14;
    MyCompareData[21].px=-2*m_Circle14;
    MyCompareData[22].px=-2*m_Circle14;
    MyCompareData[23].px=-2*m_Circle14;
    //R角
    MyCompareData[24].py=685620 - (m_p3-(m_Long/2+m_FrameStep));
    MyCompareData[24].px=-2*m_Circle14-m_Circle14/2;

    //第四边数据
    MyCompareData[25].py=3600 + m_W_H +2*m_FrameStep;
    MyCompareData[26].py=3600 + m_W_H +4*m_FrameStep;
    MyCompareData[27].py=3600 + m_W_H +6*m_FrameStep;
    MyCompareData[28].py=3600 + m_W_H +8*m_FrameStep;

    MyCompareData[25].px=-3*m_Circle14;
    MyCompareData[26].px=-3*m_Circle14;
    MyCompareData[27].px=-3*m_Circle14;
    MyCompareData[28].px=-3*m_Circle14;

    //R角
    MyCompareData[29].py=631380 - (m_p3-(m_Long/2+m_FrameStep));
    MyCompareData[29].px=-3*m_Circle14-m_Circle14/2;

    flag = m_MotionCard.Compare2DSetPrm(0,&MyPrm);
    ...
    flag = m_MotionCard.CoordinateStart(1);
    flag = m_MotionCard.CoordinateMovedForXR(1);
    flag = m_MotionCard.Compare2DData(0,30,MyCompareData,0);

    flag = m_MotionCard.ComparePort(1,11,0);
    flag = m_MotionCard.Compare2DStart(0);
   ...
   // flag = m_MotionCard.CoordinateBufDelay(1,1);//延迟1ms
    ...
    //第二边
    flag = m_MotionCard.CoordinateLineMove(1,m_p3 - (m_Width/2 + 2*m_FrameStep) + 3600,m_FrameOffset-m_W_H,-m_Circle14,m_iFrameVel,m_iFrameAcc);
    //flag = m_MotionCard.CoordinateBufDelay(1,100);
    flag = m_MotionCard.CoordinateLineMove(1,m_p3 + m_Width/2 + 3600+ 2*m_FrameStep,m_FrameOffset-m_W_H,-m_Circle14,m_iFrameVel,m_iFrameAcc);
    //第二个R角20000+2610
    //flag = m_MotionCard.CoordinateLineMove(1,m_p3 + m_Width/2 - WidthOffsetX,m_FrameOffset-WidthOffsetY-m_W_H,-m_Circle14-m_Circle14/2,vel,acc);
    flag = m_MotionCard.CoordinateLineMove(1,631395,-16863,-m_Circle14-m_Circle14/2,m_iFrameVel,m_iFrameAcc);
 //   flag = m_MotionCard.CoordinateBufDelay(1,1);//延迟1ms
   ...
   // flag = m_MotionCard.CoordinateBufDelay(1,1);//延迟1ms
    ...
    //第四边
    flag = m_MotionCard.CoordinateLineMove(1,m_p3 - (m_Width/2 + 2*m_FrameStep) + 3600,m_FrameOffset-m_W_H,-3*m_Circle14,m_iFrameVel,m_iFrameAcc);
    //flag = m_MotionCard.CoordinateBufDelay(1,100);
    flag = m_MotionCard.CoordinateLineMove(1,m_p3 + m_Width/2 + 3600+ 2*m_FrameStep,m_FrameOffset-m_W_H,-3*m_Circle14,m_iFrameVel,m_iFrameAcc);
    //第四个R角
    flag = m_MotionCard.CoordinateLineMove(1,631380,-19260,-3*m_Circle14-m_Circle14/2,m_iFrameVel,m_iFrameAcc);
//    flag = m_MotionCard.CoordinateBufDelay(1,1);//延迟1ms
    flag = m_MotionCard.CoordinateStart(1);
    ...
    short fifo=0;
    short fifocount=0;
    flag = m_MotionCard.Compare2DStatus(0,&status,&count,&fifo,&fifocount);
    ////////走到翻转机构/////
    flag = m_MotionCard.CoordinateLineMove(1,m_p4-50000,m_CylinderOffset,-3*m_Circle14,m_iFrameVel,m_iFrameAcc);
    flag = m_MotionCard.CoordinateLineMove(1,m_p4,m_CylinderOffset,-3*m_Circle14,m_iFrameVel,m_iFrameAcc);
    //开始运动
...
bool CRouteManager::IPhonePath_3()//翻转+摄像头
{
    ...
    flag = m_MotionCard.CoordinateLineMove(1,m_p2 - m_Long/2 + m_LightOpen,m_CameraOffset,-4*m_Circle14,m_iUpDownCameraVel,m_iUpDownCameraAcc);
...
bool CRouteManager::IPhonePath_4()//背面检测
{
    bool flag;
    //走到背面检测
    flag = m_MotionCard.CoordinateLineMove(1,m_p1 + m_Long/2 + m_LightOpen,m_FrontBackYOffSet,-4*m_Circle14,m_iBackVel,m_iBackAcc);
    //正面低角度
    flag = m_MotionCard.CoordinateBufIO(1,0x0080,0x0000);//触发正背检测低角度光1
    flag = m_MotionCard.CoordinateBufIO(1,0x0100,0x0000);//触发正背检测低角度光2
    flag = m_MotionCard.CoordinateLineMove(1,m_p1-m_Long/2 + 12*m_FrontBackImageHeight,m_FrontBackYOffSet,-4*m_Circle14,m_iBackVel,m_iBackAcc);
    flag = m_MotionCard.CoordinateBufIO(1,0x0001,0x0000);//触发 正背检测相机触发
    flag = m_MotionCard.CoordinateLineMove(1,m_p1 - (m_Long/2+m_LightOpen),m_FrontBackYOffSet,-4*m_Circle14,m_iBackVel,m_iBackAcc);
    flag = m_MotionCard.CoordinateBufIO(1,0x0181,0xFFFF);//关闭 正背检测相机 低角度光1 低角度光2 补光 触发
    flag = m_MotionCard.CoordinateLineMove(1,m_p1 - (m_Long/2+2*m_LightOpen),m_FrontBackYOffSet,-4*m_Circle14,m_iBackVel,m_iBackAcc);

    //正背同轴
    flag = m_MotionCard.CoordinateLineMove(1,m_p1 - (m_Long/2+m_LightOpen),m_FrontBackYOffSet,-4*m_Circle14,m_iBackVel,m_iBackAcc);
    flag = m_MotionCard.CoordinateBufIO(1,0x0040,0x0000);//开启正背同轴光
    flag = m_MotionCard.CoordinateBufIO(1,0x0200,0x0000);//触发正背检测补光
    flag = m_MotionCard.CoordinateLineMove(1,m_p1-m_Long/2,m_FrontBackYOffSet,-4*m_Circle14,m_iBackVel,m_iBackAcc);
    flag = m_MotionCard.CoordinateBufIO(1,0x0001,0x0000);//触发正背检测相机
    flag = m_MotionCard.CoordinateLineMove(1,m_p1 + m_Long/2 + m_LightOpen,m_FrontBackYOffSet,-4*m_Circle14,m_iBackVel,m_iBackAcc);
    flag = m_MotionCard.CoordinateBufIO(1,0x0241,0xFFFF);//关闭 正背检测相机触发 正背检测同轴光
    flag = m_MotionCard.CoordinateStart(1);
    flag = m_MotionCard.CoordinateMovedForXR(1);
    return flag;
}
