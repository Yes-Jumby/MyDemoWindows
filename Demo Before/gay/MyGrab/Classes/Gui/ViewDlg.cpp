...
void CViewDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SCG_VIEW_FORMAT, m_cbViewFormat);
...
BEGIN_MESSAGE_MAP(CViewDlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_SCG_VIEW_FORMAT, OnSelchangeViewFormat)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CViewDlg::OnInitDialog() 
{
   ...
   // Initialize view format combo
   char strFormat[64];
   memset(strFormat, 0, sizeof(strFormat));

   SapBuffer *pBuffer = m_pView->GetBuffer();

   if (pBuffer->IsMultiFormat())
   {
      int numPages, currentPage;
      SapFormat pageFormat[16];

      pBuffer->GetNumPages(&numPages);
      pBuffer->GetPageFormat(pageFormat);

      for (int i = 0; i < numPages; i++)
      {
         CString indexStr;
         indexStr.Format(_T(" (page %d)"), i);

         SapManager::GetStringFromFormat(pageFormat[i], strFormat);
         m_cbViewFormat.AddString(CString(strFormat) + indexStr);
      }

      pBuffer->GetPage(&currentPage);
      m_cbViewFormat.SetCurSel(currentPage);
      GetDlgItem( IDC_SCG_VIEW_FORMAT)->EnableWindow(TRUE);
   }
   else
   {
      GetDlgItem( IDC_SCG_VIEW_FORMAT)->EnableWindow(FALSE);
   }
...
void CViewDlg::OnSelchangeViewFormat() 
{
   // Update buffer page according to selected view format
   int index= m_cbViewFormat.GetCurSel();
   if (index != CB_ERR)
      m_pView->GetBuffer()->SetAllPage(index);
}
