...
BEGIN_MESSAGE_MAP(CFlatFieldDlg, CDialog)
   ...
   ON_WM_DESTROY()
   ON_BN_CLICKED(IDC_SCG_FF_LOAD_CLUSTER_MAP, OnBnClickedScgFfLoadClusterMap)
...
//#define USE_STANDARD_DEVIATION
   ...
   if (tooManyBadPixels && m_pFlatField->IsClippedGainOffsetDefects())
   {
      str.Format("The following statistics have been computed on the dark image: \n"
                 ...
                 (LPCSTR)GetAverageStr(stats),
#ifdef USE_STANDARD_DEVIATION
                 (LPCSTR)GetStdDeviationStr(stats),
#endif
                 m_RecommendedDark);
   ...
   else
   {
      ...
      if (AfxMessageBox(CString(str), MB_YESNO) != IDYES)
         return;
   }

	// Log pixel statistics
   str.Format("    Average pixel value: %s", (LPCSTR)GetAverageStr( stats));
	LogMessage(Info, str);

#ifdef USE_STANDARD_DEVIATION
   str.Format("    Standard deviation: %s", (LPCSTR)GetStdDeviationStr( stats)));
	LogMessage(Info, str);
#endif
   ...
   {
      LogMessage(Error, SapManager::GetLastStatus());
   }
}
...
void CFlatFieldDlg::BrightImage()
{
   ...
   str.Format("The following statistics have been computed on the bright image\n"
              ...
              "\nDo you want to use this image?",
              (LPCSTR)GetAverageStr( stats), 
				  (LPCSTR)GetPeakPositionStr( stats), 
				  (LPCSTR)GetNumPixelsStr( stats), 
				  (LPCSTR)GetPixelRatioStr( stats), 
				  (LPCSTR)GetLowStr( stats), 
				  (LPCSTR)GetHighStr( stats),
   ...
   LogMessage(Info, str);
   str.Format("    Highest peak position: %s", (LPCSTR)GetPeakPositionStr( stats));
   LogMessage(Info, str);
   str.Format("    Lower bound: %s", (LPCSTR)GetLowStr( stats));
   LogMessage(Info, str);
   str.Format("    Upper bound: %s", (LPCSTR)GetHighStr( stats));
   LogMessage(Info, str);
   str.Format("    Number of pixels inside bounds: %s (%s)", (LPCSTR)GetNumPixelsStr( stats), (LPCSTR)GetPixelRatioStr( stats));
   LogMessage(Info, str);
...
void CFlatFieldDlg::OnBnClickedScgFfLoadClusterMap()
{   

#if _MSC_VER >= 1500
   // Account for new 'bVistaStyle' argument for CFileDialog constructor in Visual C++ 2008 and up
   CFileDialog fileDlg(TRUE, _T("csv"), _T("clustermap.csv"), OFN_FILEMUSTEXIST | OFN_HIDEREADONLY, _T("CSV (Comma delimited) (*.csv)|*.csv||"), this, 0, TRUE);
#else
   CFileDialog fileDlg(TRUE, _T("csv"), _T("clustermap.csv"), OFN_FILEMUSTEXIST | OFN_HIDEREADONLY, _T("CSV (Comma delimited) (*.csv)|*.csv||"), this);
#endif

   if (fileDlg.DoModal() == IDOK)
   {
      CString path = fileDlg.GetPathName();
      if (m_pFlatField->SetClusterMap((CStringA)path))
      {
         CString sMsg;
         sMsg.Format(_T("Cluster map loaded with %d defective pixels"), m_pFlatField->GetClusterMapPixelCount());
         GetDlgItem(IDC_SCG_FF_CLUSTER_MAP_PIXEL_COUNT)->SetWindowText(sMsg);
      }
      else
         GetDlgItem(IDC_SCG_FF_CLUSTER_MAP_PIXEL_COUNT)->SetWindowText(_T("Invalid cluster map"));
   }
...
void CFlatFieldDlg::LogMessage(int messageType, const char* string)
{
   LogMessage(messageType, CString(string));
}
...
CStringA CFlatFieldDlg::GetAverageStr( SapFlatFieldStats& stats) const
{
	CStringA str;
...
CStringA CFlatFieldDlg::GetStdDeviationStr( SapFlatFieldStats& stats) const
{
	CStringA str;
...
CStringA CFlatFieldDlg::GetLowStr( SapFlatFieldStats& stats) const
{
	CStringA str;
...
CStringA CFlatFieldDlg::GetHighStr( SapFlatFieldStats& stats) const
{
	CStringA str;
...
CStringA CFlatFieldDlg::GetPeakPositionStr( SapFlatFieldStats& stats) const
{
	CStringA str;
...
CStringA CFlatFieldDlg::GetPixelRatioStr( SapFlatFieldStats& stats) const
{
	CStringA str;
...
CStringA CFlatFieldDlg::GetNumPixelsStr( SapFlatFieldStats& stats) const
{
	CStringA str;
