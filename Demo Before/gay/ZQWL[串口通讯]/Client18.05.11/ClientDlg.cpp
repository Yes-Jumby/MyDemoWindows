...
void CClientDlg::OnLogin() //开始做实验
{
	...
	buffer[6],buffer[7],buffer[8],buffer[9],buffer[10],buffer[11],buffer[12]&0xFF,buffer[13],buffer[14]);
	 ...
	 sprintf(buf1,"第%d次",num1);
	m_Count.SetWindowText(buf1);
	IsOpen=true;
}
...
void CClientDlg::OnTimer(UINT_PTR nIDEvent)
{
    ...
    fprintf(file0, "%s\t%s\t%d:%d:%dT%d:%d:%d:%d\n",buf1,buf,utm0.wYear,utm0.wMonth,utm0.wDay,utm0.wHour,utm0.wMinute,utm0.wSecond,utm0.wMilliseconds);
	fclose(file0);
	}
