...
std::vector<int> getY_From_Range_To_Sensor1(
...
{
	{
		...
		iconRange.loadFromIconFile("D:\\Stereo3D\\data\\sick_demo\\extra\\1",1.0);
		uint16_t* pBeg = (uint16_t*)iconRange.getData(SickCam::DataNames::RAN);
		std::vector<uint16_t> aLine(pBeg, pBeg + 2560);
		std::vector<int> res = getY_From_Range_To_Sensor1(aLine, 0, 256, false);
        ...
        cv::imshow("Camera Calibration", senIm3);//显示图片
        cv::waitKey(10);//暂停0.1S   
	}

	{
		...
		uint16_t* pBeg = (uint16_t*)iconRange.getData(SickCam::DataNames::RAN);
		std::vector<uint16_t> aLine(pBeg, pBeg + 2560);
		std::vector<int> res = getY_From_Range_To_Sensor1(aLine, 0, 832, false);
